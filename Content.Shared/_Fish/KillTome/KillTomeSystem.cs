...
public sealed class KillTomeSystem : EntitySystem
{
    ...
    private void OnPaperAfterWriteInteract(Entity<KillTomeComponent> ent, ref PaperAfterWriteEvent args)
    {
        ...
        var showPopup = false;
        var killsRemaining = ent.Comp.MaxKillCount - ent.Comp.KillCount;
        ...
        foreach (var line in lines)
        {
            ...
            // Check if kill tome has reached its kill limit
            if (ent.Comp.KillCount >= ent.Comp.MaxKillCount)
            {
                _popupSystem.PopupEntity(Loc.GetString("killtome-max-kills-reached"), ent.Owner, args.Actor, PopupType.MediumCaution);
                return;
            }
            ...
            _adminLogs.Add(LogType.Chat,
                LogImpact.High,
                $"{ToPrettyString(args.Actor)} has written {ToPrettyString(uid)}'s name in Kill Tome. Kills: {ent.Comp.KillCount}/{ent.Comp.MaxKillCount}");

            // Update remaining kills after each successful kill
            killsRemaining = ent.Comp.MaxKillCount - ent.Comp.KillCount;

            // Show remaining kills popup
            if (killsRemaining > 0)
            {
                _popupSystem.PopupEntity(Loc.GetString("killtome-kills-remaining", ("count", killsRemaining)),
                    ent.Owner, args.Actor, PopupType.Medium);
            }
        }
