...
/// </summary>
public sealed class FishCrtThemeScope : PanelContainer
{
    private readonly IFishCrtAppearanceManager _appearanceManager;
    ...
    private FishCrtEffects _effects = FishCrtEffects.HorizontalScanlines;
    private FishCrtPalettePreset _palette = FishCrtPalettePreset.Blue;

    public FishCrtPalette ResolvedPalette { get; private set; }
    internal FishCrtThemeContext ResolvedContext { get; private set; }

    public FishCrtPalettePreset Palette
    {
        get => _palette;
        set
        {
    ...
    public FishCrtThemeScope()
    {
        _appearanceManager = IoCManager.Resolve<IFishCrtAppearanceManager>();
        _resourceCache = IoCManager.Resolve<IResourceCache>();
        _stylesheet = IoCManager.Resolve<IStylesheetManager>();
        PanelOverride = _style;
        ResolvedPalette = FishCrtPalettes.Get(FishCrtPalettePreset.Blue);
        ResolvedContext = new FishCrtThemeContext(ResolvedPalette, _appearanceManager.Settings);
        RefreshTheme();
    }
    ...
    private void OnAppearanceChanged(FishCrtAppearanceSettings settings)
    {
        RefreshTheme();
    }
    ...
    {
        ResolvedPalette = Palette == FishCrtPalettePreset.Custom
            ? new FishCrtPalette(
                CustomForeground,
                CustomBackground,
                CustomBorder,
        ...
        FishCrtThemeHelpers.ApplyToDescendants(this, ResolvedContext);
    }
    ...
    private Stylesheet CreateCrtStylesheet()
    {
        ...
        {
            Element<Label>().Class(FishCrtStyleClasses.Text)
                .Prop(Label.StylePropertyFontColor, ResolvedPalette.Foreground),
            Element<Label>().Class(FishCrtStyleClasses.Heading)
                .Prop(Label.StylePropertyFontColor, ResolvedPalette.Foreground),
